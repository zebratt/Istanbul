...
class Settle extends Component {
  ...
  componentDidMount() {
    const { customerId, token, querySchemeData } = this.props;

    if (!customerId) {
      return;
    }

    querySchemeData(customerId, token, {});
  }
  ...
  render() {
    const { customerId, token, settleData } = this.props;
      ...
      <div id="Settle">
        <div className="line">
          <div className="item">
            ...
            <div className="input">
              <input value={stockCodeVal} onChange={(eve)=>{
                this.setState({
                  stockCodeVal: eve.target.value
                })
              }} maxLength={12} className="my-input" type="text" />
            </div>
          </div>
          <div className="item">
            <div className="label">股票名称</div>
            <div className="input">
              <input value={stockNameVal} onChange={(eve)=>{
                this.setState({
          ...
          <table>
            <tbody>
              ...
              <tr>
                ...
                <td>卖出成交价格</td>
                <td>
                  {modalData.sellOutPrice}
                </td>
              </tr>
              <tr>
                <td>点买股票数量</td>
                <td>
                  {modalData.buyStockQuantity}
                </td>
                <td>成交股票数量</td>
                <td>
                  {modalData.clinchStockQuantity}
                </td>
              </tr>
              <tr>
                <td>点买单的状态</td>
              ...
              <tr>
                <td>创建时间</td>
                <td>
                  {modalData.creationTime}
                </td>
                <td>所需保证金</td>
                <td>
                  {modalData.requiredMargin}
                </td>
              </tr>
              <tr>
                <td>止盈金额</td>
                <td>
                  {modalData.profitAmount}
                </td>
                <td>止损金额</td>
                <td>
                  {modalData.lossAmount}
                </td>
              </tr>
              <tr>
                <td>综合费</td>
                <td>
                  {modalData.totalCharges}
                </td>
                <td>可持仓天数</td>
                <td>
                  {modalData.positionDays}
                </td>
              </tr>
              <tr>
                <td>递延总费用</td>
                <td>
                  {modalData.deferredCharges}
                </td>
                <td>欠费金额</td>
                <td>
                  {modalData.owedSum}
              ...
              <tr>
                <td>每日延迟费</td>
                <td>
                  {modalData.dayilyDeferredCharge}
                </td>
                <td>平仓时间</td>
                <td>
                  {modalData.liquidationTime}
                </td>
              </tr>
              <tr>
                <td>总亏盈</td>
                <td>
                  {modalData.sumProfitLoss}
                </td>
                <td>客户亏盈</td>
                <td>
                  {modalData.customerProfitLoss}
                </td>
              </tr>
              <tr>
                <td>投资人亏盈</td>
                <td>
                  {modalData.investorProfitLoss}
                </td>
                <td>递延条件</td>
                <td>
                  {modalData.deferredCondition}
                </td>
              </tr>
              <tr>
                <td>点买额度</td>
...
const mapStateToProps = state => {
  const { Settle, Home } = state;

  return Object.assign({}, Settle, Home);
};
